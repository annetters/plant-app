...
- **Plant fields**: common name, scientific name, cultivar, flower color, bloom window (start and end month/day, year-independent — see `CONTEXT.md`), sun/shade requirement, mature height/spread, USDA hardiness zone, deciduous/evergreen, native/non-native, reference photo(s) (at least one expected), recurring care-task templates.
...
- **Seams**: there is no existing codebase to integrate against, so internally the application is a single seam. ADR-0002 adds three *external* ones, each of which should sit behind its own adapter: a geocoder (address → coordinates), a tile server (aerial imagery), and a parcel service (lot boundaries). Each fails in a way the application has to handle rather than propagate — missing tiles are served as grey placeholders with HTTP 200 and cannot be detected from the status code, parcel coverage is per-jurisdiction and must distinguish "outside our coverage" from "no parcel at this point", and cross-origin tiles taint the canvas so that every later export throws. All three are optional-path dependencies: the blank-canvas fallback must work with none of them reachable.
