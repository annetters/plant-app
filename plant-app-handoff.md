...
**Not independently re-verified by this session that the fix renders correctly on web** — the commit went in on the user's direct instruction ("Commit them all"), not after an explicit "I confirmed it renders now." Two things worth keeping in mind if this comes up again:
1. **This does NOT retroactively fix any already-broken photo** — a HEIC file already sitting in storage from before this fix stays broken forever; that's expected, not a new bug. Removing and re-adding it is the only way to fix an existing one.
