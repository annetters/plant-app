/**
 * Mirrors `packages/domain/src/usdaChecklist.ts` — the Deno edge runtime can't
 * import that npm workspace package (same arrangement as `property.ts` and
 * `create-property`). **Keep the two in sync**; the domain copy is the one
 * with the test suite, including the fixture that checks this parser against
 * USDA's own answer for all 2,186 taxa it publishes both name forms for.
 */
