 ...
 * photographed/drawn base map, with no address and no geocoding at all.
 ...
 * with the row it ends up belonging to.
 */
export interface PropertyCreateWithBaseMapInput {
  id: string
  name: string
  baseMapSource: 'photo' | 'drawn'
  baseMapPhotoPath: string | null
  baseMapDrawing: PropertyInput['baseMapDrawing']
...
export class PropertiesRepository {
  ...
  /**
   * Creates a Property directly from a photographed/drawn base map — the
   * privacy-motivated up-front alternative to `create()`'s address-driven
   * aerial flow (see CONTEXT.md's Property entry). No Edge Function, no
   * geocoding, no Esri call: unlike `create()`, this touches no external
   * adapter, same as `updateBaseMap` below.
   */
